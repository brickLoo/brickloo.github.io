...
PGD 方法其实仍有许多待改进的地方。在这个工作中，作者就对 PGD 提出了进一步改进，并融合了同期其他两个不使用梯度信息的对抗样本生成方法，整理成一个多方法的模型性能评估策略 AutoAttack，并成为了当时的 SOTA 方法。
...
- 另外，参考训练过程中优化步长逐步减小的策略，作者这里也提出了一套随着对抗样本生成过程逐渐减小单次扰动大小 $\alpha$ 的方案，缓解了这个超参数大了不精，小了太慢的问题；
