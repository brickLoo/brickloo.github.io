...
ANN 的意义在于将模式识别模型视为结合了线性和非线性处理的单元的组合。在这个基础上，模式识别模型蓬勃发展，诞生出前馈神经网络、反馈神经网络和图网络等网络结构。同时前馈神经网络中又有感知机和卷积神经网络等。这里，我们进一步地详细探讨一种最基础的模型——感知机。

正如前文描述激活函数时提及的那样，前馈神经网络中的每一个神经元都可以用以下数学公式表示：
$$g(x)=f(w^\intercal\phi(x))$$
其中 $f$ 为非线性的激活函数，$\phi(x)$ 则是来自其他神经元的输入或直接就是原始输入 $x$ ，这个输入相对于原始输入 $x$ 可以是非线性的。
...
$$L(w)=-g(x)y$$
但是我们依旧要进行分类正误的判断。因为如果我们继续使用已经分类正确的样本进行训练和参数更新，模型就会进一步放大参数的数值，使得模型更加容易放大一些非重点信息的影响，因此可能对于噪声更加敏感，影响模型的泛化能力。
...
径向基网络也是一种前馈神经网络，因此也可以使用数学语言描述为 $g(x)=f(w^\intercal\phi(x))$。然而与感知机不同的是，径向基网络不是直接对数据点进行线性划分，而认为数据点是“一丛一丛”的，并使用数据点到某些特定坐标的距离信息来实现数据类别的划分。因此，它能够比较好地对各种线性或非线性的情况进行分类。其中，衡量距离信息的函数称为**径向基函数**。

简而言之，径向基网络的工作流程如下：
- 数据经过任意方式的处理进入隐含层或不经过处理，输入给多个径向基函数评估到每个中心点的距离，这些距离信息有助于将弧线可分的数据变为线性可分的数据；
- 通过进一步对距离信息进行线性组合，就可以实现非线性的分类。

径向基函数的选择有多种，而比较常用的是高斯函数，因为它只在距离中心点附近的某一邻域内响应显著，而函数值随着与中心点的距离的增大而呈单调递减趋势，并逐渐趋近于 $0$。

相比起感知机需要反复进行线性划分和激活的方式，径向基网络不是尝试用多条直线拟合曲线，而是使用距离信息实现弯曲的效果，并通过径向基函数的参数控制弯曲的程度，因此在相对复杂的任务上，径向基网络只需要更简单的模型内部结构和更短的训练时间。然而，也是由于径向基函数的存在，模型在前向传播时计算速度会相对更慢。

### XOR Example

还是可以用经典的异或问题作为例子来理解径向基网络的工作流程。如果我们希望能够将 $(0,1)$ 和 $(1,0)$ 的情况与 $(0,0)$ 和 $(1,1)$ 分离开，我们不妨：
- 在 $(0,1)$ 和 $(1,0)$ 处分别设置中心点，到每一个点的距离评估结果可以作为一个隐含层神经元的输出，这时一共使用两个隐含层。
- 在合理选择径向基函数参数的情况下，$(0,1)$ 和 $(1,0)$ 到各自中心点的距离可以为 $1$，因此到两个中心点的距离之和大于 $1$；而剩余两点 $(0,0)$ 和 $(1,1)$ 到两个中心点的距离之和可以小于 $1$。
- 这时，问题就变回了线性可分问题，很简单也有很多种方法实现了。

### Parameter Determination

相比起感知机，除了线性组合部分的参数，非线性处理部分的参数也是不固定的，也需要进行训练。通用的训练思路如下：
1. 使用 K-means 等聚类方法找到合适的中心点。当使用高斯函数作为径向基函数的时候，这一步确定的是参数 $\mu$。
2. 利用数据的方差等信息进一步确定宽度范围。当使用高斯函数作为径向基函数的时候，这一步确定的是参数 $\sigma$。
3. 这时非线性处理部分已经确定，可以进一步通过梯度下降等方法确定线性组合部分的参数。